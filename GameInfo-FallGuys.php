...
if (isset($_POST["comment"])) {
    ...
    if (isset($_POST['insert'])) {
        ...
        $sql = "INSERT INTO MakesComments (commentTime, content, playerID, gameID) VALUES ((now()),'$content','$id',3)";
    }
    if ($conn->query($sql) === TRUE) {
        header('location:GameInfo-FallGuys.php');
    } else {
...
<div id="Centered">
    ...
    <div class="Header">
        <img src="ProjectIMG/Game/FallGuys.jpg"/>
    </div>
    ...
    <div class="MainBox">
        ...
        <div class="Content">
            ...
            <?php
            $conn = OpenCon();
            $query = "SELECT G.gameType, G.latestVersion, P.companyName FROM Game AS G, PublishUpdateGame AS Publish, Producer AS P WHERE G.gameID = 3 AND Publish.gameID = G.gameID AND Publish.companyID = P.companyID";
            ...
            $query = "SELECT C.content, P.username, C.commentTime FROM MakesComments AS C, Player AS P WHERE C.gameID = 3 AND C.playerID = P.playerID";
            $result = $conn->query($query);
            ...
            <?php
            $conn = OpenCon();
            $query = "SELECT * FROM HasPlayer_Game_Accomplishment WHERE playerID = '$id' and gameID = '3'";
            $result = $conn->query($query);
            if ($result->num_rows > 0) {
                echo '<form method="POST" action="GameInfo-FallGuys.php">';
            ...
            <?php
            $conn = OpenCon();
            $query = "SELECT timeSpent, rating FROM HasPlayer_Game_Accomplishment WHERE gameID = 3 AND playerID = '$id'";
            $result = $conn->query($query);
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
            ...
            $query = "SELECT H.accomplishmentID, A2.reward, A2.accomplishLevel, H.obtainedDate FROM HasPlayer_Game_Accomplishment AS H, Accomplishment AS A, Accomplishment2 AS A2 WHERE H.gameID = 3 AND playerID = '$id' AND H.accomplishmentID = A.accomplishmentID AND A.accomplishLevel = A2.accomplishLevel";
            $result = $conn->query($query);
            if ($result->num_rows > 0) {
                echo "<table border=1>";
