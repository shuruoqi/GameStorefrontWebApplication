...
if (isset($_POST["comment"])) {
    ...
    if (isset($_POST['insert'])) {
        ...
        $sql = "INSERT INTO MakesComments (commentTime, content, playerID, gameID) VALUES ((now()),'$content','$id',8)";
    }
...
<div id="Centered">
    ...
    <div class="MainBox">
        ...
        <div class="Content">
            ...
            <?php
            $conn = OpenCon();
            $query = "SELECT G.gameType, G.latestVersion, P.companyName FROM Game AS G, PublishUpdateGame AS Publish, Producer AS P WHERE G.gameID = 8 AND Publish.gameID = G.gameID AND Publish.companyID = P.companyID";
            ...
            $query = "SELECT C.content, P.username, C.commentTime FROM MakesComments AS C, Player AS P WHERE C.gameID = 8 AND C.playerID = P.playerID";
            $result = $conn->query($query);
            ...
            <?php
            $conn = OpenCon();
            $query = "SELECT * FROM HasPlayer_Game_Accomplishment WHERE playerID = '$id' and gameID = '8'";
            $result = $conn->query($query);
            if ($result->num_rows > 0) {
                echo '<form method="POST" action="GameInfo-StardewValley.php">';
